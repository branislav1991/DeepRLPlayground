...
for epoch in range(num_epochs):
  ...
  with torch.no_grad():
    ...
    for _ in range(replay_buffer_size):
      ...
      for t in range(max_env_steps):
        ...
        replay_buffer.append((observation, action, new_observation, reward, done))
  ...
  replay_buffer = np.stack(sorted(replay_buffer, key=lambda replay: abs(replay[3]), reverse=True))
  p = np.array([unusual_sample_factor ** i for i in range(len(replay_buffer))])
  p = p / sum(p)
  ...
  for _ in range(1000):
    ...
    sample_idxs = np.random.choice(np.arange(len(replay_buffer)),size=batch_size, p=p)
    sample = [replay_buffer[idx] for idx in sample_idxs]

    # tensors from replay buffer
    observations = torch.tensor([r[0] for r in sample], dtype=torch.int64).view((-1)) 
    observations = F.one_hot(observations, env.observation_space.n).float()
    actions = torch.tensor([r[1] for r in sample], dtype=torch.int64).view(-1,1)
    new_observations = torch.tensor([r[2] for r in sample], dtype=torch.int64).view((-1))
    new_observations = F.one_hot(new_observations, env.observation_space.n).float()
